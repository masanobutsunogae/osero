...
public class OurPlayer extends ap26.Player {
  ...
  static final String MY_NAME = "pl10";
  ...
  float maxSearch(Board currentBoard, float alpha, float beta, int depth) {
    ...
    if (depth == 0 || depth == 2) {
      ...
      EvalResult bestResult =
          new EvalResult(firstMove, minSearch(firstBoard, alpha, beta, depth + 1));

      // もし合法手が1個しかなかったらここで終わり
      if (moves.size() == 1) {
        if (depth == 0) this.move = bestResult.move();
        return bestResult.score();
      }

      // firstMoveの評価値をalphaとして使う
      float initialAlpha = Math.max(alpha, bestResult.score());
      final float capturedAlpha = initialAlpha;
      final float capturedBeta = beta;
                  ...
                  nextMove ->
                      CompletableFuture.supplyAsync(
                          () -> {
                            ...
                            float childValue =
                                minSearch(nextBoard, capturedAlpha, capturedBeta, depth + 1);
  ...
  float minSearch(Board currentBoard, float alpha, float beta, int depth) {
    ...
    if (depth == 1) {
      Move firstMove = moves.get(0);
      Board firstBoard = currentBoard.clone().placed(firstMove);
      float initialBeta = maxSearch(firstBoard, alpha, beta, depth + 1);
