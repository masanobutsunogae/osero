 ...
 * オセロ盤面の評価関数。
 *
 * <p>マスごとの重み行列で盤面を評価する（黒視点）。 終局時は score() に大きな係数を掛けて、確実な勝敗を最優先する。
 */
public class MyEval {

  /** マスごとの重み行列。M[row][col] でアクセスする。 角・辺は大きな正の値、角の斜め隣 (X マス) は負の値。 */
  ...
  public float value(Board board) {
    ...
    float position = (float) IntStream.range(0, LENGTH).mapToDouble(k -> cellScore(board, k)).sum();
