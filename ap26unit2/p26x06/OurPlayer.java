...
public class OurPlayer extends ap26.Player {
  ...
  List<Move> order(List<Move> moves, boolean descending) {
  List<Move> ordered = new ArrayList<>(moves);

  ordered.sort((m1, m2) -> {
    float v1 = eval.cellScore(this.board, m1.getIndex());
    float v2 = eval.cellScore(this.board, m2.getIndex());
